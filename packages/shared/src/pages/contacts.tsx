...
import { api, type ContactList, type ImportResult } from "../api";

export type SaveFile = (filename: string, bytes: Uint8Array, mime: string) => Promise<void>;
...
async function downloadTemplate(saveFile: SaveFile): Promise<void> {
  ...
  await saveFile("contacts_template.xlsx", data, XLSX_MIME);
}
...
function ExcelTab({ saveFile }: { saveFile: SaveFile }) {
  ...
  async function handleDownload() {
    setDlSaving(true);
    try {
      await downloadTemplate(saveFile);
      showToast({ message: "template saved", type: "success" });
    } catch {
      showToast({ message: "could not save template", type: "error" });
    } finally {
      setDlSaving(false);
    }
  }
...
export function Contacts({ saveFile }: { saveFile: SaveFile }) {
  ...
  return (
    <div className="page-content">
      ...
      {tab === "excel" && <ExcelTab saveFile={saveFile} />}
